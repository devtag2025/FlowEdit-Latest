...
export default function PriceCard({
  ...
  return (
    <div
      className={`relative overflow-hidden px-10 py-8 border rounded-lg bg-white/40 flex flex-col gap-6 ${
        glow ? "shadow-xl" : ""
      }`}
    >
      <Image
      ...
      <div className="relative z-10 text-center">
        ...
        <ul className="space-y-5 mt-5 text-left">
          {features.map((f, i) => (
        ...
        {/*
          ✅ No auth check needed here — user doesn't need to be logged in.
          They pay via Stripe first, then sign in with Google on the dashboard.
          Clicking always goes straight to Stripe checkout.
        */}
        <button
          onClick={onSubscribe}
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white mt-9 py-2 rounded-xl transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
